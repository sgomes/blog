...
import gulpMarked from './util/marked.mjs';
...
let env = new nunjucks.Environment(
	new nunjucks.FileSystemLoader([__dirname + '/src/', __dirname + '/.build/']),
)
	.addFilter('formatDate', (str, format) =>
		moment(str, dateFormat).format(format),
	)
...
export async function compressTouch() {
	gulp
		.src(`./.build/assets/images/touch/*.png`, { encoding: false })
		.pipe(imagemin())
...
function copyRoot() {
	return src(['./src/*'], { nodir: true, encoding: false }).pipe(
		dest('./.dist'),
	);
}

function copyAssets() {
	return src(['./src/assets/**/*'], { base: './src', encoding: false }).pipe(
		dest('./.dist'),
	);
}

function copyPostAssets() {
	return src(['./src/posts/**/*', '!./src/posts/**/*.md'], {
		base: './src',
		encoding: false,
	}).pipe(dest('./.dist'));
}
...
function buildPostData() {
	return src(['./src/posts/**/index.md'], { base: './src' }).pipe(
...
function buildThePosts({ production = false } = {}) {
	return src(['./src/posts/**/index.md'], { base: './src' })
		...
		.pipe(gulpMarked([highlight, gfmHeadingId()]))
		.pipe(
			data((file) => {
				const headings = getHeadingList();
				const toc = `
					<ul id="table-of-contents">
						${headings.map(({ id, raw, level }) => (level === 2 ? `<li><a href="#${id}" class="h${level}">${raw}</a></li>` : '')).join('\n')}
					</ul>
				`;
				return { headings };
			}),
...
const buildDocumentsDev = series(
	buildPostData,
	buildPostsDev,
	buildPagesDev,
	buildFeed,
);
const buildDocumentsProd = series(
	buildPostData,
	buildPostsProd,
	buildPagesProd,
	buildFeed,
...
export const buildDev = series(
	clean,
	scss,
	parallel(buildDocumentsDev, copyStatic, createIcons),
);
export const buildProd = series(
	clean,
	scss,
	parallel(buildDocumentsProd, copyStatic, createIcons),
);
